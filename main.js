...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // THE FIX: Changed eyeYRatio to 0.50 (dead center) and eyeDistRatio to 0.12 (tiny).
    // This entirely stops the engine from aggressively cropping/zooming in on the original photo.
    const aligner = new FaceAligner({ eyeYRatio: 0.50, eyeDistRatio: 0.12 });
    ...
    function realignAllFrames() {
        ...
        showProgress('Re-aligning Mode…', 0);
        frames = [];
    ...
    async function handleFiles(fileList) {
        ...
        showProgress('Preparing images…', 0);

        for (let i = 0; i < files.length; i++) {
            const file = files[i];
            
            // Explicit processing text update for the user
            setProgressLabel(`Scanning face: ${file.name}`);
            await sleep(10); 
    ...
    function drawGuides() {
        ...
        // THE FIX: The blue rings now dynamically multiply by the currentZoom so they perfectly track the eyes
        const eyeDist = W * 0.12 * currentZoom; 
        const cx = W / 2;
        const cy = H * 0.50; 
        const r = 18;
    ...
    // Default vs Smart Mode Switches
